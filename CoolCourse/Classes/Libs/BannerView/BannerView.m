...
- (instancetype)initWithFrame:(CGRect)frame noDataImageName:(NSString *)imageName {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _pageControl.currentPageIndicatorTintColor = CommonBlue;
...
- (void)scroll {
    ...
    [_collectionView setContentOffset:CGPointMake([[UIScreen mainScreen] bounds].size.width * scrollIndex, 0) animated:YES];
...
//        NSString *imgUrlStr = [NSString stringWithFormat:@"%@?x-oss-process=image/resize,m_fill,h_%f,w_%f/quality,q_75",url, self.mj_h * 0.5, self.mj_w * 0.5];
//        [imageURLs addObject:[NSURL URLWithString:imgUrlStr]];
    ...
    if ([urls count] <= 1) {
        ...
        [_collectionView setContentOffset:CGPointMake([[UIScreen mainScreen] bounds].size.width, 0)];
        self.index = 1;
    ...
    // 缓存
//    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
//    [defaults setObject:[NSMutableArray arrayWithArray:urls] forKey:@"bannerCache"];
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(BannerItem *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    [cell.imageView yy_setImageWithURL:_urls[indexPath.row] placeholder:nil];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    self.index = scrollView.contentOffset.x / [[UIScreen mainScreen] bounds].size.width;
    if (_index == 0) {
        [scrollView setContentOffset:CGPointMake([[UIScreen mainScreen] bounds].size.width * (_urls.count - 2), 0) animated:NO];
